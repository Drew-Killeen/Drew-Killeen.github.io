...
{
  var output = "", first, text = plaintext.split(" ");
...
{
  var output = "", last, text = plaintext.split(" ");
  for(var i = 0; i < text.length; i++) {
    if(/way$/.test(text[i])) {
...
function railFenceDecrypt(text,key) {
	if(text.length < 1){ alert("please enter some plaintext (letters only)");
	return;
  }
